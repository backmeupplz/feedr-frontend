...
v-card(
  flat,
  height='100vh',
  style='position: absolute; top: 0; bottom: 0; left: 0; right: 0; width: 100%'
)
  v-row.pr-1
    v-col.scrollable.scroller.border__top(
      cols='0',
      sm='4',
      md='3',
      lg='2',
      ...
      div(
        v-if='bot.chats',
        v-for='(chat, i) in sortedChats',
        :key='chat._id',
        v-observe-visibility='(isVisible, entry) => visibilityChanged(isVisible, entry, i)'
      )
        v-list-item(
          @click='openChat(chat)',
          @click.prevent='setMessageSendFocus',
          active-class='active',
          :class='{ "v-list-item--active": chatActivated(chat) }'
        )
          v-list-item-content
          ...
          v-list-item-action
            v-list-item-action-text(
              ...
              v-text='formatDateHM(new Date(chat.lastMessage.updatedAt))'
            )
    ...
    v-col(
      ...
      md='9',
      lg='10',
      :class='{ "pa-4": $vuetify.breakpoint.xsOnly, "pa-0 pr-4 chatview": $vuetify.breakpoint.smAndUp }'
    )
      v-navigation-drawer(
        v-model='chatnav',
        absolute,
        ...
        v-list(nav, two-line)
          ...
          v-list-item-group(active-class='text--accent-8')
            div(
              ...
              v-list-item(
                @click='openChat(chat); chatnav = !chatnav',
                @click.prevent='setMessageSendFocus',
                active-class='active',
                :class='{ "v-list-item--active": chatActivated(chat, selectedChat) }'
              ) 
                v-list-item-content
                  v-list-item-title {{ getChatName(chat).name }}
                  v-list-item-subtitle(v-if='chat.lastMessage') 
                  ...
                  v-list-item-subtitle(v-if='curbot.botType === "feed"') 
                    ...
                    v-icon(
                      x-small,
                      v-else-if='getBotType(chat) === "telegram"'
                    ) mdi-telegram
                    v-icon(x-small, v-else-if='getBotType(chat) === "vk"') mdi-vk
            ...
            template(v-slot:activator='{ on }')
              span(v-on='on') 
                span(v-if='getChatName(chat).type === "feed"')
                  ...
                  v-icon(
                    small,
                    v-else-if='getChatName(chat).botType === "telegram"'
                  ) mdi-telegram
      ...
      .headline.pa-4.text-center(
        v-if='!chat && curbot && curbot.chats && curbot.chats.length'
      ) {{ $t("chat.select") }}
      .headline.pa-4.text-center(
        v-else-if='!chat && curbot && !curbot.chats.length && curbot.botType !== "feed"'
      ) {{ $t("chat.invite") }}
        a(
          v-if='bot.botType === "telegram"',
          :href='"https://t.me/" + curbot.username',
          target='_blank'
        ) 
          | @{{ curbot.username }}
        div(v-else)
          | @{{ curbot.username }}
      .headline.pa-4.text-center(
        v-else-if='curbot.chats && !curbot.chats.length && curbot.botType === "feed"'
      ) 
        div(v-if='$store.state.bots.length > 1') {{ $t("chat.nochats") }}
        div(v-else) {{ $t("noBots") }}
      v-container.border__right.border__left(
        v-else,
